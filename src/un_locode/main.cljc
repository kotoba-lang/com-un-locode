(ns un_locode.main
  "Kotodama WASM entrypoint for the Un_locode clean-room actor (L5) — Clojure port.
  ...
  facts (`un_locode.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "un_locode")
(def tier "L5")
...
(def entity-specs
  [{:entity "Location"           :plural "locations"           :id-prefix "unlocode_loc"
    :fields [:countryCode :locationCode :name :nameWoDiacritics :function :status :subdivision :iataCode :latitude :longitude]
    :required [:countryCode :locationCode]
    :coerce {:latitude :float :longitude :float}
    :refs {}}
   {:entity "FunctionClassifier" :plural "functionclassifiers" :id-prefix "unlocode_fun"
    :fields [:code :definition]
    :required [:code :definition]
    :coerce {}
    :refs {}}
   {:entity "StatusCode"         :plural "statuscodes"         :id-prefix "unlocode_sta"
    :fields [:code :definition]
    :required [:code :definition]
    :coerce {}
    :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"un_locode.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "un_locode-compat" :tier tier :entities entities} 200])
...
(defn start! [] :un_locode-compat/ready)
